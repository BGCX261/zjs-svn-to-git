/*=============================================================================
    ...
    There are several terms that need to be defined:
      ...
      Package
          A Java-like hierarchical naming structure that maps to a folder. It is
          best practice to keep packages equivalent to namespaces, though this
          is not enforced. Packages can be nested as with Java and are then often
          called "sub-packages". Package names are separated by dots, like
          namespaces and also like Java, to form a Package Path.

      Package Path
          A sequence of package names separated by dots. For example, take the
          string "foo.bar.bif". These are always absolute. In other words, "foo"
          is the top-level package with a "bar" sub-package, in turn with a "bif"
          sub-package.

      Root Package
          A top-level (non-nested) package. For example, the root package of
          "foo.bar.bif" is "foo".

      Module
          A member of a package in a single JavaScript (.js) file. There can be
          multiple modules in a JavaScript file, but modules do not span files.
          A file that contains multiple modules is called a Bundle.

      Bundle
          A JavaScript file that contains multiple modules. These are typically
          produced by a build process that combines individual files into one
          for efficiency.

      Module Name
          The leaf file name, sans extent and path. For example, the name of
          this module is "import" (no ".js" and no "zjs").

      Module Path
          The concatenation of the package and module name separated by a dot
          character ("."). For example, the module path of this module is
          "zjs.import".
...
/**
This class provides the logic to manage modules for a package. The interface
that must be satisfied to be used as "Duck Type Compatible" substitute are:

    string[] getRequiredModules (string scriptText);

    string makeUrl (string modpath);

This implementation of getRequiredModules uses a simple regex to parse out any
$requires statements in the source. This does not respect block comments, but
does understand "//" commented lines.
...
*/
PackageManager : $class(null,
{
    /**
    Stores the given prefix and (optional) suffix for forming URL's.

    @param prefix The URL prefix text (required but may be empty).
    @param suffix The (optional) suffix text.
    */
    ...
    */
    ...
    @param scriptText The JavaScript source text.
    @return The array of module paths of the required modules.
    */
    ...
    @param modpath The module path to convert to a relative path.
    @return The relative path for the given module path.
    */
    makeRelPath : function (modpath)
    {
    ...
    @param modpath The module path to map to a URL.
    @return The URL to use to retrieve the specified module's JS file.
    */
...
Module : $class(null,
{
    ...
    */
    State : $static($enum
    (
        // The module was just initialized and needs to be moved to either
        // LOADING or READY (depending on usage).
        "INIT",

        // The module is loading. There are two phases here: 1) Ajax and
        // 2) Script tag. Phase 1 is always performed, while phase 2 is only
        // used when debugging on compatible browsers (!Safari).
        "LOADING",

        // The module is loaded into the DOM and live. This is the case for
        // code loaded via explicit script tags and is also the eventual
        // state for dynamically loaded code (if successful).
        "READY",

        // The module failed to load. Consult the module object's error
    ...
    */
    ...
    @return True if this module is ready.
    */
    isReady : function ()
    {
    ...
    */
    load : function (onready)
    {
        ...
        this.xact = zjs.ajax(src, { ondone : this.onLoad.bind(this) });
    },
    ...
    */
    ...
    @param tr The AjaxTransaction object (same as this.xact).
    */
    onLoad : function (tr)
    {
        $assert(this.state === zjs.Module.State.LOADING);
    ...
    */
    onPreReady : function ()
    {
        ...
        //
        if (imp.config.debug && imp.config.canUseScriptSrcLoad)
        {
            var src = this.pkgmgr.makeUrl(this.path);
...
Importer : $singleton(null, function ()
{
    ...
    */
    ...
    return ( // blank line is important (JavaScript grammar ambiguity)
    {
        /**
        ...
        */
        ctor : function ()
        {
            ...
            // Indexed by top-level package name (e.g., "foo").
            this.packages = { };

            // Modules that are loaded indexed by module path. The value is
            // a Module object.
            //
            this.modules = { };
        ...
        */
        ...
        @return The PackageManager for the given module or null.
...
@param id The (optional) ID of the new <script> element.
...
/**
Loads a sequence of modules and calls an optional completion callback. The
signature of this method is:

    $import(modpaths [, modpaths]* [, onload]?)

The first argument is one or more module paths. This can be a single module
path string (e.g., "foo.bar.bif") or an array of module path strings (e.g.,
["foo.bar.bif", "bar.fiz.baz"]). In the case of an array, all of the modules
are requested in parallel. When all are loaded, the next batch is started or
the user callback (if any) is made.

Any number of module paths arguments can be passed. When the first batch has
been loaded, the next batch is started. This continues through all of the given
module paths.

When all module paths have been loaded, the onload callback is made (if one was
given). For example:
...
Batch 1 (foo.bar.bif and bar.fiz.baz) is requested in parallel. When complete,
the next batch (bif.boo.zip and jazz.rip.fifi) is requested in parallel. Then
the single module bonzi.booze is requested. Finally, when that module loads, the
user's onload method is called.

This kind of loading is not necessary for modules that use $requires since they
are automatically loaded in their transitive dependency order. For loading code
that does not use $requires, however, its dependency needs are not expressed in
the code.
*/
function $import ()
{
    var imp = zjs.Importer.getInstance();
    var onload, n = arguments.length;
